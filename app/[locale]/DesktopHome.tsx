...
function DesktopHomeInner({ materials }: { materials: Material[] }) {
  ...
  const { isLoggedIn, isAuthLoading, userId, userEmail, userName, userInitial, avatarUrl, profile, authProviders,
    ...
    <div style={{ display: "flex", height: "100vh", background: "#f8f4f4", overflow: "hidden", position: "relative" }}>
      <aside style={{ width: sbOpen ? SB_OPEN : SB_CLOSED, transition: "width 0.22s ease", background: "transparent", display: "flex", flexDirection: "column", flexShrink: 0, overflow: "visible", zIndex: 10 }}>
        ...
        <div style={{ padding: "10px 6px", flexShrink: 0, position: "relative" }}>
          ...
          <div ref={userIconRef} onClick={() => { if (isAuthLoading) return; if (!isLoggedIn) { setGuestMenuOpen(!guestMenuOpen); } else { setUserMenuOpen(!userMenuOpen); } }} style={{ display: "flex", alignItems: "center", gap: 8, padding: sbOpen ? "6px 10px" : "6px 0", justifyContent: sbOpen ? "flex-start" : "center", borderRadius: 10, cursor: isAuthLoading ? "default" : "pointer", background: userMenuOpen ? "rgba(163,192,255,0.1)" : "transparent" }}>
            <div style={{ width: 32, height: 32, borderRadius: "50%", background: isAuthLoading ? "rgba(200,180,220,0.25)" : "linear-gradient(135deg,#f4b9b9,#e49bfd)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "white", flexShrink: 0, overflow: "hidden" }}>
              {!isAuthLoading && (avatarUrl ? <img src={avatarUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : userInitial)}
            </div>
            ...
            {sbOpen && !isAuthLoading && isLoggedIn && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#bbb" strokeWidth="2"><path d="M18 15l-6-6-6 6" /></svg>}
          </div>
