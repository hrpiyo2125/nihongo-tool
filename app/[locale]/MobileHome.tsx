    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "white", overflow: "hidden" }}>
      ...
      <header style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50, height: 56, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 16px", background: scrolled ? "white" : "transparent", borderBottom: scrolled ? "0.5px solid rgba(200,170,240,0.2)" : "none", transition: "background 0.2s" }}>
        ...
        <div style={{ position: "relative" }}>
          <button
            onClick={() => { if (isAuthLoading) return; isLoggedIn ? setMyPageOpen(true) : setGuestMenuOpen(v => !v); }}
            style={{ width: 36, height: 36, borderRadius: "50%", background: isAuthLoading ? "rgba(200,180,220,0.25)" : "linear-gradient(135deg,#f4b9b9,#e49bfd)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, color: "white", cursor: isAuthLoading ? "default" : "pointer", overflow: "hidden", padding: 0 }}
          >
            {!isAuthLoading && (isLoggedIn && avatarUrl ? <img src={avatarUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : isLoggedIn ? userInitial : "?")}
          </button>
          {guestMenuOpen && (
            <>
