        ...
        <div style={{ padding: "20px 18px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[
              ...
              { label: "枚数", value: material.pageCount != null ? `${material.pageCount}枚` : "－" },
            ].map(({ label, value }) => (
      ...
      <header style={{ flexShrink: 0, background: "linear-gradient(135deg,#f4b9b9 0%,#e49bfd 45%,#a3c0ff 100%)", padding: "0 16px", height: 52, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 6 : 8, flexShrink: 0 }}>
          <div style={{ position: "relative" }}>
            <button onClick={handleFavClick} onMouseEnter={() => setFavHover(true)} onMouseLeave={() => setFavHover(false)} style={{ display: "flex", alignItems: "center", gap: isMobile ? 4 : 6, height: isMobile ? 28 : 34, padding: isMobile ? "0 9px" : "0 12px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.55)", background: favHover ? "rgba(255,255,255,0.22)" : "transparent", cursor: "pointer" }}>
              {!isLoggedIn ? (
              ...
              <span style={{ fontSize: isMobile ? 11 : 12, fontWeight: 600, color: "white", whiteSpace: "nowrap" }}>{isLoggedIn ? (isFav ? "保存済み" : "お気に入り") : "お気に入り"}</span>
            </button>
            <LockTooltip type="favorite" visible={activeTooltip === "favorite"} onClose={() => setActiveTooltip(null)} onOpenAuth={openAuth} />
          </div>
          <div style={{ position: "relative" }}>
            <button onClick={handleDownloadClick} onMouseEnter={() => setDlHover(true)} onMouseLeave={() => setDlHover(false)} style={{ display: "flex", alignItems: "center", gap: isMobile ? 4 : 6, height: isMobile ? 28 : 34, padding: isMobile ? "0 10px" : "0 16px", borderRadius: 8, border: "none", background: dlHover ? "rgba(255,255,255,0.85)" : "white", cursor: "pointer", boxShadow: "0 1px 4px rgba(0,0,0,0.1)" }}>
              <svg width={isMobile ? 12 : 14} height={isMobile ? 12 : 14} viewBox="0 0 24 24" fill="none" strokeWidth="2"><path d="M12 3v13M7 11l5 5 5-5" stroke="#333" /><path d="M4 20h16" stroke="#333" /></svg>
              <span style={{ fontSize: isMobile ? 11 : 12, fontWeight: 700, color: "#333", whiteSpace: "nowrap" }}>ダウンロード</span>
            </button>
          </div>
        </div>
      </header>
