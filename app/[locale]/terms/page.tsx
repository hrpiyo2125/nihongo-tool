...
・解約後は次回更新日をもって有料プランの利用権限が失効します
・解約後にダウンロード済みのデータの利用は認められません
・プランのダウングレード後は、変更後のプランに対応した教材のみご利用いただけます
・解約予約中であっても、プランの変更（アップグレード・ダウングレード）は可能です。その際、解約予約を取り消すか維持するかを選択いただけます`,
  },
  {
    title: "サブスクリプション・都度購入",
    content: `有料プランは月次サブスクリプション形式です。解約はマイページからいつでも行えます。解約後は次回更新日までサービスをご利用いただけます。料金の返金は原則として行っておりません。

【都度購入】
ライトプラン以上のご利用者は、対象教材を1点あたり350円（税込）で単品購入することも可能です。購入した教材はサブスクリプションの有無に関わらずダウンロード可能ですが、本規約に定める利用範囲の制限は引き続き適用されます。

支払いはStripe社を通じて処理されます。お支払い情報に問題が発生した場合、プランがFreeプランに移行することがあります。その際は個別にご連絡の上、適切に対応いたします。`,
  },
  {
    title: "サービスの変更・停止・終了",
