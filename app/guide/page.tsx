...
type Tip  = { emoji: string; title: string; desc: string; link?: string; linkLabel?: string };
type Card = { emoji: string; title: string; items: string[] };
...
const startTips: Tip[] = [
  { emoji: "💡", title: "最初におすすめの教材は？", desc: "「ひらがな練習シート」や「ひらがなかるた」は初めての方に人気です。まずここから試してみてください。", link: "/", linkLabel: "教材一覧を見る →" },
...
const chooseTips: Tip[] = [
  { emoji: "🔍", title: "迷ったときは「内容×方法」で絞り込む", desc: "教材一覧では「ひらがな × かるた」のように2軸で絞り込めます。どちらかだけでも絞り込めるので、まず学習内容か方法のどちらか気になる方から試してみてください。", link: "/", linkLabel: "教材一覧で絞り込む →" },
  { emoji: "❤️", title: "お気に入りに保存しておく", desc: "「使いたいかも」と思った教材はハートボタンでお気に入りに保存できます（要ログイン）。あとでまとめて確認できて便利です。" },
];
...
const useTips: Tip[] = [
  { emoji: "🎮", title: "「楽しい」が一番の近道", desc: "テストや練習だけでなく、かるた・ゲーム・うたなど楽しめる教材を混ぜることで、子どもが日本語を「好き」になるきっかけを作れます。" },
  { emoji: "📅", title: "週のルーティンに組み込む", desc: "「月曜は練習シート、金曜はかるた」のように曜日で教材の種類を変えると飽きにくく継続しやすくなります。" },
];

// ===== もっと活用したい =====
const moreTips: Tip[] = [
  { emoji: "📂", title: "ダウンロード履歴を活用する", desc: "過去にダウンロードした教材はマイページの履歴からすぐ再ダウンロードできます。印刷のたびに探し直す手間が省けます。" },
  { emoji: "❤️", title: "お気に入りリストを整理する", desc: "学習テーマや季節ごとにお気に入りをまとめておくと、授業・学習の計画が立てやすくなります。" },
  { emoji: "🔓", title: "サブスクプランで全教材を使い放題に", desc: "無料プランでは一部の教材のみご利用いただけます。サブスクプランに登録すると体系的なカリキュラム・全教材が使い放題になります。", link: "/plan", linkLabel: "プランを見る →" },
  { emoji: "📬", title: "新着教材をチェックする", desc: "トップページの「新着」タブで最新の教材を確認できます。定期的に新しい教材が追加されます。" },
  { emoji: "💬", title: "リクエスト・フィードバックを送る", desc: "「こんな教材がほしい」「使ってみた感想」などはお問い合わせページから送ってください。教材制作の参考にします。", link: "/contact", linkLabel: "お問い合わせ →" },
];
...
function TipItem({ tip }: { tip: Tip }) {
  return (
    <div style={{ background: "linear-gradient(135deg,rgba(244,185,185,0.08),rgba(163,192,255,0.08))", border: "0.5px solid rgba(200,170,240,0.25)", borderRadius: 14, padding: "20px 24px" }}>
      <div style={{ fontSize: 15, fontWeight: 700, color: "#444", marginBottom: 8, display: "flex", alignItems: "center", gap: 8 }}>
        <span>{tip.emoji}</span>{tip.title}
      </div>
...
function CardItem({ card }: { card: Card }) {
  return (
    <div style={{ background: "white", border: "0.5px solid rgba(200,170,240,0.2)", borderRadius: 14, padding: "22px 24px" }}>
      <div style={{ fontSize: 15, fontWeight: 700, color: "#7a50b0", marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
