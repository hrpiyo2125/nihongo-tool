...
export default function MobilePlanModal({ onClose }: Props) {
  return (
    <div
      onClick={onClose}
    ...
    >
      <div
        ...
        style={{ background: "#f8f4f4", borderRadius: "20px 20px 0 0", width: "100%", maxHeight: "90vh", overflowY: "auto", position: "relative", fontFamily: "'Hiragino Sans', 'Yu Gothic', 'Noto Sans JP', sans-serif" }}
      >
        {/* ヘッダー */}
        <div style={{ position: "sticky", top: 0, zIndex: 10, height: 56, display: "flex", alignItems: "center", padding: "0 16px", borderBottom: "0.5px solid rgba(200,170,240,0.2)", background: "#f8f4f4", gap: 12 }}>
          <button onClick={onClose} style={{ fontSize: 22, color: "#aaa", background: "none", border: "none", cursor: "pointer", lineHeight: 1, padding: 0 }}>‹</button>
          <span style={{ fontSize: 16, fontWeight: 700, color: "#333" }}>料金プラン</span>
        </div>

        {/* コンテンツ */}
        <div style={{ padding: "24px 16px 48px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 20, marginBottom: 48 }}>
            {plans.map((plan) => (
              <div
                key={plan.key}
                style={{
              ...
              >
                ...
                <h2 style={{ fontSize: 16, fontWeight: 800, color: "#444", marginBottom: 8, marginTop: 0 }}>{plan.name}</h2>
          ...
          <div style={{ background: "white", borderRadius: 16, border: "0.5px solid rgba(200,170,240,0.2)", padding: "28px 24px", marginBottom: 32 }}>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: "#444", marginBottom: 16, marginTop: 0 }}>よくある質問</h2>
            <dl style={{ margin: 0 }}>
          ...
          <div style={{ textAlign: "center" }}>
            <a href="/plan" style={{ display: "inline-block", fontSize: 14, padding: "12px 40px", borderRadius: 24, background: "linear-gradient(135deg,#f4b9b9,#e49bfd)", color: "white", textDecoration: "none", fontWeight: 700 }}>
              プランを選んで始める →
            </a>
          </div>
        </div>
