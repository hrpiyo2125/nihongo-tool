...
const ANIM = `@keyframes rfFadeUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}`;
...
export default function RequestForm({ onSubmit, onBack }: Props) {
  ...
  async function handleConfirm() {
    ...
    setSubmitting(true);
    onSubmit(selectedTopic, summary);
  }
  ...
  if (phase === "done") {
    return (
      <>
        <style>{ANIM}</style>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: "32px 16px", animation: "rfFadeUp 0.28s ease-out both" }}>
          ...
          <button
            onClick={resetForm}
            style={{ padding: "9px 24px", borderRadius: 20, border: "none", background: "linear-gradient(135deg,#f4b9b9,#e49bfd)", color: "white", fontWeight: 700, fontSize: 13, cursor: "pointer", animation: "rfFadeUp 0.28s 0.1s ease-out both" }}
          >
            別の件を送る
          </button>
        </div>
  ...
  if (phase === "confirm") {
    return (
      <>
        <style>{ANIM}</style>
        <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: "16px", animation: "rfFadeUp 0.26s ease-out both" }}>
          <p style={{ fontSize: 13, color: "#9b6ed4", fontWeight: 700, margin: 0 }}>送信内容の確認</p>
          <div style={{ background: "white", borderRadius: 12, border: "1.5px solid rgba(155,110,212,0.3)", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 6, animation: "rfFadeUp 0.26s 0.06s ease-out both" }}>
          ...
          <div style={{ display: "flex", gap: 8, animation: "rfFadeUp 0.26s 0.12s ease-out both" }}>
  ...
  if (phase === "input" && selectedTopic) {
    ...
    return (
      <>
        <style>{ANIM}</style>
        <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: "16px", animation: "rfFadeUp 0.26s ease-out both" }}>
          ...
          {schema.map((field, fi) => (
            <div key={field.id} style={{ display: "flex", flexDirection: "column", gap: 8, animation: `rfFadeUp 0.24s ${0.05 * (fi + 1)}s ease-out both` }}>
          ...
          <button
            onClick={handleNext}
            disabled={!isReady}
            style={{ padding: "11px 0", borderRadius: 20, border: "none", background: isReady ? "linear-gradient(135deg,#f4b9b9,#e49bfd)" : "#e5e5e5", color: isReady ? "white" : "#bbb", cursor: isReady ? "pointer" : "default", fontSize: 13, fontWeight: 700, animation: `rfFadeUp 0.24s ${0.05 * (schema.length + 1)}s ease-out both`, transition: "background 0.2s, color 0.2s" }}
          >
            確認する
          </button>
        </div>
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6, padding: "16px" }}>
        {onBack && (
          <button onClick={onBack} style={{ background: "none", border: "none", cursor: "pointer", padding: "0 0 8px", color: "#9b6ed4", fontSize: 12, textAlign: "left", animation: "rfFadeUp 0.22s ease-out both" }}>← 戻る</button>
        )}
        <p style={{ fontSize: 12, color: "#9b6ed4", fontWeight: 700, margin: "0 0 6px", letterSpacing: 0.3, animation: "rfFadeUp 0.22s ease-out both" }}>カテゴリを選んでください</p>
        {FORM_TOPICS.map((t, i) => (
          <button key={t} style={{ ...outlineBtn(), animation: `rfFadeUp 0.24s ${0.06 * (i + 1)}s ease-out both` }} onClick={() => handleTopicSelect(t)}>{t}</button>
        ))}
      </div>
    </>
  );
